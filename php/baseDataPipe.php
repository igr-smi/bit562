 ...
 * an author, an essay id, etc., automatically create the needed sql for
 * selects, insertions, updates, and deletes.
 *
 * The instructions needed to trigger a particular line of sql comes from
 * the browser through the REQUEST.  "object_ID", "queryType",
 * "pipe choice", "tableName", and like are established at the browser
 * controller and get passed to the server through the request.
 * Control information and form data get passed in the same request.
 * Control information and data are combined into a single object on the
 * browser.
 *
 * Additional information for creating sql comes from tablemaps.  Specific
 * requests will often be handled through special purpose data pipes
 * created as extensions to BaseDataPipe.  Many queries are handled through
...
class BaseDataPipe {
    ...
    function __construct($mapMgr, $dataManager) {
        ...
        // Look first to see if there is a map specific to this table and
        // queryType.
        $this->map = $mapMgr->findMatchingMaps($this->tableName,
            $this->queryType);
        ...
        /*+ TODO::location=baseDataPipe.php::task=When deciding on numeric or alpha as a dataType, handle more numeric cases than just int.::author=Ed Anderson */

        // If no maps were present in the map table, then get the column
        // headings from the data table itself.  Assume the browser side
        // and the table side are the same in this case.
        if ( count($this->map) === 0 ) {
    ...
    /*+ TODO::location=BaseDataPipe.php::task=Think through the logic of masterid table handling and the possible failure of an insert into the masterid table::author=Ed Anderson */
    ...
    public function execute() {
        ...
        // Manage inserts, updates, and selects.
    ...
    protected function update() {
        ...
        for ( $i=0, $len=count($this->map); $i<$len; $i+=1 ) {
            ...
            // Analyze each field to insure that it has data.
            // If data is missing, the field does not get added to
            // the update.
            $browser = $this->map[$i]->get('browser');
    ...
    // If the tableName is given in the REQUEST, use it.  Otherwise, 
    // look up the object_ID in the masterid table and find the name of the 
    // table it resides in.
    ...
    // Look through the tableMaps to see if one of them is a groupoid.  
    ...
    /****************************************************************
     * Data returned in the resultset stemming from the executed sql is 
     * passed to resultToXML for packaging with XML tags.  The package is 
     * the same format MySQL produces when you save a query directly to a 
     * file.
     ****************************************************************/
    public function resultToXML($recordSet) {
        ...
        case "insert" :
            ...
            $XML .= "<field name=\"object_ID\">".$this->object_ID.
                "</field>";
            $XML .= "<field name=\"result\">".$successFlag."</field>";
            $XML .= "</row>";
            break;

            // Package data retrieved in XML tags for transmission back
            //  to the browser.
            // Format:
            // <row tableName="enotes">
            //   <field name="object_ID">xsaew-triee-uoirw-kdixi</field>
            //   <field name="note">
            //     This could be a note of considerable length.
            //   </field>
            // </row>
        case "keyword" :
        ...
        case "select" :
            ...
            while ($row = $recordSet->fetch(PDO::FETCH_ASSOC)) {
                ...
                for ( $i=0, $len=count($this->map)-$offset; $i<$len; $i+=1 ) {
                    $XML .= "<field name=\"".
                        $this->map[$i]->get('browser').
                        "\">".
                        $this->db->deScrub(
                            $row[$this->map[$i]->get('dbCol')]
                        );
                    $XML .= "</field>";
                }
